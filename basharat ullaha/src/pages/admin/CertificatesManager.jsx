...
import { FileText, Plus, Trash2, Edit2, Loader2, Save, X } from 'lucide-react';
...
export default function CertificatesManager() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const data = await getCollectionData('certificates');
        setItems(data);
      } catch (err) {
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editingId) {
        await updateCollectionItem('certificates', editingId, formData);
        setItems(items.map(i => i.id === editingId ? { ...i, ...formData } : i));
      } else {
        const id = await addCollectionItem('certificates', formData);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await deleteCollectionItem('certificates', id);
    ...
    <div className="space-y-10">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
          <div>
            <h2 className="font-sans font-medium text-2xl text-slate-100 tracking-tight">Certificates</h2>
