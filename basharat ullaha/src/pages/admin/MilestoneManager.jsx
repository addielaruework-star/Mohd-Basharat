...
export default function MilestoneManager() {
  ...
  const [formData, setFormData] = useState({ title: '', year: '', description: '', type: '' })
  const [saving, setSaving] = useState(false)
  const [toast, setToast] = useState({ show: false, message: '', type: 'success' })

  // Load collection when tab changes — keeps Firestore collections unchanged
  useEffect(() => {
    async function load() {
      setLoading(true)
      try {
  ...
  const openEditor = (item = null) => {
    ...
    if (item) {
      setFormData({ title: item.title, year: item.year, description: item.description || '', type: item.type || '' })
      ...
      setFormData({
        ...
        description: '',
        type: currentTab?.hasType ? 'Certification' : '',
      })
      setEditingId(null)
    }
    setIsEditing(true)
  ...
  return (
    <div className="space-y-8 animate-enter">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
        <div>
          <h2 className="font-sans font-medium text-2xl text-slate-100 tracking-tight">Milestone</h2>
          <p className="text-sm text-slate-400 mt-1.5">Manage achievements, awards &amp; certificates across all milestone categories.</p>
        </div>
        {!isEditing && (
          <button
            onClick={() => openEditor()}
          ...
          >
            <div className="p-6 border-b border-slate-800 flex justify-between items-center bg-[#c9a84c]/5">
              <h3 className="text-[0.7rem] font-bold text-slate-400 uppercase tracking-[0.15em]">
                {editingId
                  ? `Edit ${currentTabMeta?.label.slice(0, -1) || 'Record'}`
                  : `Create New ${currentTabMeta?.label.slice(0, -1) || 'Record'}`}
              </h3>
            ...
            <form onSubmit={handleSave} className="p-6 lg:p-8 space-y-6">
              ...
              {/* Certificate type field — only shown on Certificates tab */}
              {currentTabMeta?.hasType && (
                <div className="space-y-2">
                  <label className="block text-[0.75rem] font-medium text-slate-400">Certificate Type</label>
                  <input
                    type="text" required
                    placeholder="e.g. Certification, Participation, Recognition"
                    value={formData.type}
              ...
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  className="w-full px-4 py-3 bg-slate-950/50 border border-slate-800 rounded-xl focus:outline-none focus:border-[#c9a84c]/50 text-slate-100 transition-all text-[0.9rem] resize-none"
                />
              </div>
            ...
            {loading ? (
              ...
              <div className="bg-slate-900/80 backdrop-blur-xl border border-slate-800 rounded-2xl p-24 text-center flex flex-col items-center justify-center shadow-lg shadow-black/20 animate-enter">
                <div className="w-16 h-16 bg-[#c9a84c]/5 rounded-full flex items-center justify-center mx-auto mb-6 border border-slate-800/50">
                  {currentTabMeta && <currentTabMeta.icon className="text-slate-500" size={24} />}
                </div>
                <h3 className="font-medium text-slate-100 mb-2 text-[0.95rem]">No {currentTabMeta?.label} Found</h3>
                    ...
                    <li key={item.id} className="p-6 hover:bg-slate-800/20 transition-colors group">
                      <div className="flex justify-between items-start gap-6">
                        <div className="flex-1">
                          ...
                          <p className="text-[0.85rem] text-slate-400 leading-relaxed" style={{ whiteSpace: 'pre-line' }}>{item.description}</p>
                        </div>
