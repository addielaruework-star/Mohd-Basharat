...
import { Trophy, Plus, Trash2, Edit2, Loader2, Save, X } from 'lucide-react';
...
export default function AchievementsManager() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const data = await getCollectionData('achievements');
        setItems(data);
      } catch (err) {
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editingId) {
        await updateCollectionItem('achievements', editingId, formData);
        setItems(items.map(i => i.id === editingId ? { ...i, ...formData } : i));
      } else {
        const id = await addCollectionItem('achievements', formData);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await deleteCollectionItem('achievements', id);
    ...
    <div className="space-y-10">
      ...
      <div className="animate-enter">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
          <div>
            <h2 className="font-sans font-medium text-2xl text-slate-100 tracking-tight">Achievements</h2>
