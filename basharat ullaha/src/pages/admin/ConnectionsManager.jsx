...
export default function ConnectionsManager() {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        const data = await getCollectionData('connections');
        setItems(data);
      } catch (err) {
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editingId) {
        await updateCollectionItem('connections', editingId, formData);
        setItems(items.map(i => i.id === editingId ? { ...i, ...formData } : i));
      } else {
        const id = await addCollectionItem('connections', formData);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await deleteCollectionItem('connections', id);
